...
public class ThreadManager {
    ...
    public static Thread getPageObjectThread(String testName, int numberOfExecutions, PageNavigationTypes type) {
        ...
        try {
            ...
            logger.log(LoggerManagerLevel.FATAL, String.format("Class [%s] not found ! [%s]", className, e), PropertiesManager.getEmailNotification());
        } catch (NoSuchMethodException e) {
            logger.log(LoggerManagerLevel.FATAL, String.format("Method not found ! [%s]", e), PropertiesManager.getEmailNotification());
        } catch (InstantiationException e) {
            logger.log(LoggerManagerLevel.FATAL, String.format("Could not instantiate [%s] ! [%s]", className, e), PropertiesManager.getEmailNotification());
        } catch (IllegalAccessException e) {
            logger.log(LoggerManagerLevel.FATAL, String.format("Illegal access ! [%s]", e), PropertiesManager.getEmailNotification());
        } catch (InvocationTargetException e) {
            logger.log(LoggerManagerLevel.FATAL, String.format("Invocation target exception ! [%s]", e), PropertiesManager.getEmailNotification());
        }
        return null;
    }
}
